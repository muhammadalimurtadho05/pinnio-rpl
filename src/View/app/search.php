      ...
      <div class="page-header">
        <div class="feed-col">
          ...
          <form id="searchForm" class="mb-4 d-flex gap-2">
            <input type="text" id="searchInput" class="form-control bg-dark text-white border-secondary" placeholder="Search by name or username" value="<?= htmlspecialchars($data['query'] ?? '') ?>">
            ...
            <?php else: ?>
              <div class="list-group list-group-flush bg-transparent">
                <?php foreach ($data['users'] as $u): ?>
                  <div class="list-group-item bg-transparent border-secondary text-white d-flex align-items-center justify-content-between py-3">
                    <div class="d-flex align-items-center gap-3">
                      <div style="width:44px;height:44px;border-radius:50%;background:var(--pin-card);border:1.5px solid var(--pin-border);display:flex;align-items:center;justify-content:center;font-size:22px;">
                      ...
                      <div>
                        <h6 class="mb-0" style="font-weight:700;"><?= htmlspecialchars($u['name'] ?: $u['username']) ?></h6>
                        <small style="color:var(--pin-muted);">@<?= htmlspecialchars($u['username']) ?></small>
                      </div>
                    </div>
                    <!-- Follow button will be implemented in the follow feature -->
